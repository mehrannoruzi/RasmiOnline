"use strict"; $(window).on("load", function () { setTimeout(function () { $(".page-loader").fadeOut() }, 500) }),
...
$(document).ready(function () {
    ...
    $("body").on("click", "[data-ma-action]", function (b) {
        ...
        switch (d) {
            case "search-open": $(".search").addClass("search--toggled");
                break;
            case "search-close": $(".search").removeClass("search--toggled");
                break;
            case "aside-open": e = c.data("ma-target"), c.addClass("toggled"), $(e).addClass("toggled"), $(".content, .header").append('<div class="ma-backdrop" data-ma-action="aside-close" data-ma-target=' + e + " />");
                break;
            case "aside-close": e = c.data("ma-target"), $('[data-ma-action="aside-open"], ' + e).removeClass("toggled"), $(".content, .header").find(".ma-backdrop").remove();
                break;
            //case "aside-open": e = c.data("ma-target"), c.addClass("toggled"), $(e).addClass("toggled"),  c.data('ma-action', 'aside-close');
            //    break;
            //case "aside-close": e = c.data("ma-target"), c.removeClass("toggled"), $(e).removeClass("toggled"), $(".content, .header").find(".ma-backdrop").remove(), c.data('ma-action', 'aside-open');
            //    break;
            case "fullscreen": a(document.documentElement);
                break;
            case "print": window.print();
                break;
            case "clear-localstorage": swal({ title: "Are you sure?", text: "This can not be undone!", type: "warning", showCancelButton: !0, confirmButtonColor: "#3085d6", confirmButtonText: "Yes, clear it", cancelButtonText: "No, cancel" }).then(function () { localStorage.clear(), swal("Cleared!", "Local storage has been successfully cleared", "success") });
                break;
            case "login-switch": e = c.data("ma-target"), $(".login__block").removeClass("active"), $(e).addClass("active");
                break;
            case "notifications-clear": b.stopPropagation(); var f = $(".top-nav__notifications .listview__item"), g = f.length, h = 0; c.fadeOut(), f.each(function () {
                var a = $(this);
...
var setActiveMenu = function () {
    ...
    $('aside ul.navigation a:not([href="#"])').each(function () {
        if (cUrl.toLowerCase().indexOf($(this).attr('href')) > -1)
        {
            if ($(this).closest('ul').hasClass('navigation'))
                $(this).parent().addClass('navigation__sub--active navigation__sub--toggled');
            else 
                $(this).parent().addClass('navigation__active').closest('ul').parent().addClass('navigation__sub--active navigation__sub--toggled');
            return false;
        }
...
String.prototype.cThSeperator = function () {
    return this.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}

$(document).ajaxError(function (event,e) {
    if (e.status == "403" || e.status == "408")
        window.location.href = "/OAuth/Index?activePanel=signin";
    else if (e.status == "401")
        window.location.href = "/OAuth/UnAuthorized";
